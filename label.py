...
def generate_labels(directory, label, prefix):
    """
    Generate a CSV file with filenames and their corresponding labels.
    
    :param directory: Directory containing the audio files.
    :param label: Label to assign (1 for gunshot, 0 for no gunshot).
    :param prefix: Prefix to identify the class (e.g., 'gunshot' or 'no_gunshot').
    """
    with open(f'{prefix}_labels.csv', mode='w', newline='') as file:
...
generate_labels(balanced_dir, 1, 'gunshot')
generate_labels(balanced_dir, 0, 'no_gunshot')

print("Labels generated and saved.")
